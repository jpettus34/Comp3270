...
public class Autocomplete {
   ...
   public static class TrieAutocomplete implements Autocompletor {
      ...
      private void add(String word, double weight) {
         ...
         Node curr = myRoot;//store root node
         //navigate to word node or add the nodes until the word is added.   
         for(int i = 0; i < word.length();i++) {
            ...
            char currChar = word.charAt(i);//store current character
                        
            //If it has no child with the next char, add a new node.
            if (!curr.children.containsKey(currChar)){
				curr.children.put(currChar, new Node(word.charAt(i), curr, weight));
            curr.myInfo = curr.myInfo + currChar;
			   }
          ...
          //edge case- adding a word that has already been added
          if(curr.isWord && curr.myWeight > weight){
			   curr.myWeight = weight;
            curr.setWeight(weight);
            while(curr != null) { 
               double maxWeight = -1;//weights cannot be negative
               //navigate back up the tree and reset the mySubtreeMaxWeight accordingly.
         ...
         //make sure to set to correct values.
			   curr.isWord = true;
            curr.setWord(word);
            curr.setWeight(weight);
			   curr.myWeight = weight;
      ...
      public Iterable<String> topMatches(String prefix, int k) {
         ...
         Node curr = myRoot;
         for(int i = 0; i < prefix.length();i++) { //navigate to prefix node.
         ...
         //create max heap priority queue and priotity queue to hold the top nodes.
         PriorityQueue<Node> pq = new PriorityQueue<Node>(k, new Node. ReverseSubtreeMaxWeightComparator());
         PriorityQueue<Node> min = new PriorityQueue<Node>(k);
         pq.add(curr);
         
         //sort through the nodes in pq adding to min when necessary.
         while(!pq.isEmpty()){
            curr = pq.remove();
            if(min.size() == k && min.peek().myWeight < curr.myWeight && curr.isWord) {
               min.poll();
               min.add(curr);
            }
            else if(curr.isWord){
            ...
            for(Character ch : curr.children.keySet()) {
               pq.add(curr.children.get(ch));
            }
         }
         
         ArrayList<String> answers = new ArrayList<String>();
         
         Node[] nodes = new Node[min.size()];
		...
		for(int i = 0; i < nodes.length && i < k; i++){
			...
			answers.add(nodes[index].getWord());
		}
      ...
      public String topMatch(String prefix) {
         ...
         Node curr = myRoot;
         //navigate to prefix node, return empty string if not found.
         ...
         if(curr.mySubtreeMaxWeight == curr.getWeight() && curr.isWord) {
            return curr.getWord();
         }
         //navigate down child path using mySubtreeMaxWeight  
         while(curr.mySubtreeMaxWeight != curr.myWeight && !curr.isWord) {
            for(Character ch: curr.children.keySet()) {
               if(curr.children.get(ch).mySubtreeMaxWeight == curr.mySubtreeMaxWeight) {
